...
float rain(in PS_Input PSInput){
   float c = 0.0;
   if( fract( PSInput.pos.y ) == 0.0 || fract( PSInput.pos.y ) == 0.5 ) {
      float eff = splash( fract( PSInput.pos.xz ) );
      c = mix( 0.0, eff, max( PSInput.uv1.y - 0.8, 0.0 ) * 5.0 );
   }
   return c;
}
